...
class MTIBlueLevelsAdjustment: MTILevelsAdjustmentFilter {


    var minimumLevel: Float = 0 {
        willSet {
            minimumColor = simd_float3(0, 0, newValue)
        }
    }
    var middleLevel: Float = 1 {
        willSet {
            middleColor = simd_float3(1, 1, newValue)
        }
    }
    var maximumLevel: Float = 1 {
        willSet {
            maximumColor = simd_float3(1, 1, newValue)
        }
    }

    var minimumOutputLevel: Float = 0 {
        willSet {
            minOutputColor = simd_float3(0, 0, newValue)
        }
    }

    var maximumOutputLevel: Float = 1 {
        willSet {
            maxOutputColor = simd_float3(1, 1, newValue)
        }
    }
